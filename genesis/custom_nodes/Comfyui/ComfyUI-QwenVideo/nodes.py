...
def find_ffmpeg_binary(binary_name):
    """
    查找 ffmpeg/ffprobe 二进制文件的完整路径
    
    Args:
        binary_name: 'ffmpeg' 或 'ffprobe'
    
    Returns:
        完整路径字符串,如果找不到则返回原始名称
    """
    ...
    # 如果是 Windows,尝试带 .exe 扩展名的版本
    if os.name == 'nt':
        found_path = shutil.which(binary_name_with_ext)
        if found_path:
            return found_path
    ...
    for path in common_paths:
        full_path = os.path.join(path, binary_name_with_ext if os.name == 'nt' else binary_name)
        if os.path.isfile(full_path):
            return full_path
    
    # 如果都找不到,返回原始名称(可能在 PATH 中但 shutil.which 没找到)
    return binary_name
        ...
        return {
            "required": {
                "api_key": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "placeholder": "请输入你的 OpenRouter API Key (从 https://openrouter.ai/ 获取)"
                }),
                "num_frames": ("INT", {
                    "default": 6,
                    "min": 2,
    ...
    def extract_frames(self, video_path, num_frames):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                print(f"[QwenVideo] 警告: ffprobe 执行失败: {error_msg}")
                duration = None
            else:
                duration = float(result.stdout.strip())
                print(f"[QwenVideo] ✓ 视频时长: {duration:.2f} 秒")
        except FileNotFoundError:
            print(f"[QwenVideo] ✗ 错误: 找不到 ffprobe")
            print(f"[QwenVideo] 请确保已安装 ffmpeg 并将其添加到系统 PATH 环境变量中")
            print(f"[QwenVideo] 下载地址: https://ffmpeg.org/download.html")
            print(f"[QwenVideo] 当前尝试的路径: {ffprobe_path}")
            duration = None
        except ValueError as e:
            print(f"[QwenVideo] 警告: 无法解析视频时长: {e}")
            duration = None
        except Exception as e:
            print(f"[QwenVideo] 警告: 获取视频时长时出错: {e}")
            duration = None
        ...
        if duration:
            ...
            for i in range(1, num_frames + 1):
                ...
                try:
                    ...
                    print(f"[QwenVideo] ✗ 错误: 找不到 ffmpeg")
                    print(f"[QwenVideo] 请确保已安装 ffmpeg 并将其添加到系统 PATH 环境变量中")
                    print(f"[QwenVideo] 下载地址: https://ffmpeg.org/download.html")
                    print(f"[QwenVideo] 当前尝试的路径: {ffmpeg_path}")
                    raise
                except subprocess.CalledProcessError as e:
                    print(f"[QwenVideo] ✗ ffmpeg 执行失败 (帧 {i}): {e}")
                    if e.stderr:
                        print(f"[QwenVideo] 错误信息: {e.stderr.decode('utf-8', errors='ignore')}")
                    raise
