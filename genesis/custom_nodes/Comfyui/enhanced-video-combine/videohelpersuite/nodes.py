    ...
    def combine_video(
        ...
        **kwargs
        ...
        if isinstance(images, torch.Tensor) and images.size(0) == 0:
            return ((save_output, []), original_images)
        ...
        if format_type == "image":
            ...
            if meta_batch is None or meta_batch.has_closed_inputs:
                ...
                return {"ui": {"unfinished_batch": [True]}, "result": ((save_output, []), original_images)}
            ...
            if a_waveform is not None:
                ...
                mux_args = [ffmpeg_path, "-v", "error", "-n", "-i", file_path,
                            "-ar", str(int(audio['sample_rate'])), "-ac", str(int(channels)),
                            "-f", "f32le", "-i", "-", "-c:v", "copy"] \
                            + video_format["audio_pass"] \
                            + apad + ["-shortest", output_file_with_audio_path]

                wave = audio['waveform'].squeeze(0).transpose(0,1).contiguous()
                wave_np = wave.detach().cpu().numpy().astype(np.float32, copy=False)
                if not np.isfinite(wave_np).all():
                    wave_np = np.nan_to_num(wave_np, nan=0.0, posinf=0.0, neginf=0.0)
                np.clip(wave_np, -1.0, 1.0, out=wave_np)
                if not np.isfinite(wave_np).all():
                    wave_np = np.zeros_like(wave_np, dtype=np.float32)
                if wave_np.size == 0:
                    silent_len = max(1, int(min_audio_dur * int(audio['sample_rate'])))
                    wave_np = np.zeros((silent_len, int(channels)), dtype=np.float32)
                audio_data = wave_np.tobytes()
        ...
        return {"ui": {"gifs": [preview]}, "result": ((save_output, output_files), original_images)}
