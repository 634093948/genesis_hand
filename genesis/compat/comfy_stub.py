...
class ModelPatcher:
    ...
    def __getattr__(self, name):
        """Forward unknown attributes to the model"""
        if name in ['model', 'load_device', 'offload_device', 'patches', 'model_options', 
                    'backup', 'object_patches', 'object_patches_backup', 'weight_inplace_update']:
...
class BaseModel:
    ...
    def __init__(self, model_config=None, model_type=None, device=None, **kwargs):
        ...
        self.load_device = device if device else torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.offload_device = torch.device("cpu")
        
        # Add latent_format attribute (required by WanVideo)
        # Will be set by model_config if available
        if model_config and hasattr(model_config, 'latent_format'):
            self.latent_format = model_config.latent_format
        else:
            # Create a default latent format
    ...
    def __getattr__(self, name):
        ...
        # If model has the attribute, return it
...
# Create comfy.taesd.taesd submodule (required by latent_preview.py)
taesd_submodule = ModuleType('comfy.taesd.taesd')
taesd_submodule.TAESD = TAESD
taesd_module.taesd = taesd_submodule

# Also add TAESD to main module for compatibility
...
sys.modules['comfy.taesd.taesd'] = taesd_submodule  # Add submodule
...
# Create server module for PromptServer
server_module = ModuleType('server')

class BinaryEventTypes:
    """Binary event types for server communication"""
    PREVIEW_IMAGE = 1
...
class PromptServer:
    ...
    def send_sync(self, event, data, sid=None):
        """Stub for sending sync messages"""
        # Silently ignore preview messages in non-UI mode
        pass
    ...
    def queue_updated(self):
        """Stub for queue update notification"""
        pass

server_module.PromptServer = PromptServer
server_module.BinaryEventTypes = BinaryEventTypes

# Auto-create instance so it's always available
